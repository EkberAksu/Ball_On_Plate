...
int score[2]={0,0};
int kale2X=15,kale2Ya =110,kale2Yb=210;
...
void getDatas() {
    while (1) {
        ...
        if (n > 0) {
            ...
            i++;
            if (ch[0] == 'x') {
                buf[num] = 0;   /* always put a "null" at the end of a string! */
                if (i == 8) {
...
/*FOR WINDOWS
#ifndef defined(__linux__) || defined(__FreeBSD__)
...
osg::MatrixTransform *buildJoint8() {
    ...
    mt->setMatrix(xRot);
    osg::Geode *geode_3 = new osg::Geode;
    osg::ShapeDrawable *shape1 = new osg::ShapeDrawable(new osg::Box(osg::Vec3(-10.8f, 0.0f,5.5f), width, 1.5, length),hints);
...
osg::MatrixTransform *buildJoint14() {
    ...
    mt->setMatrix(xRot);
    osg::Geode *geode_3 = new osg::Geode;
    osg::ShapeDrawable *shape1 = new osg::ShapeDrawable(new osg::Box(osg::Vec3(-5.5f, 0.0f, -10.8f), 1.5, width, length ),hints);
    shape1->setColor(osg::Vec4(0.5f, 0.5f, 0.9f, 1.0f));
...
//--------------------------------------------Text------------------------------------------------
    ...
    /*color = osg::Vec4(0.9f, 0.9f, 0.9f, 1.0f);
    osgText::Text* Goal = createTextColored( osg::Vec3(510.0f, 500.0f, 0.0f), color,
                                             "Goalllllllllllllllllll!!!", 15.0f);

    goalGeode->addDrawable(Goal);
    goalGeode->setNodeMask(0x0fffff);*/
...
//-----------------------------------------------layer/buttons----------------------------------------
    ...
    //CreateThread(NULL, 0, thread_func, NULL, 0, 0);//lİNUX
    int i;
